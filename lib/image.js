...
module.exports = function (file, index, list, images, settings, opt) {
    var gen = new Generator(file, index, list, images, settings, opt);
    return gen.css;
};

function Generator(file, list, images, settings) {
    ...
    util2.map(default_settings, function (key, value) {
...
Generator.prototype = {
    ...
    after: function (image, arr_selector, direct, scale, list) {
        ...
        if (this.settings.hash) {
            ...
            ext = "_" + hash.digest('hex').slice(0, 10) + ext;
        }

        // 这里有一个关键，就是要生成的图片的路径

        var imgOutPath = path.join(this.settings.imgRealOutputDir, this.settings.cssFileName + ext);
        util2.write(imgOutPath, imgBuffer);
        var cssImageRelativePath = path.relative(this.settings.cssRealOutputDir, this.settings.imgRealOutputDir);
        cssImageRelativePath = path.join(cssImageRelativePath, this.settings.cssFileName + ext).replace(/\\/g, "/");
        ...
        if (this.settings.ie_bug_fix) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                this.css += arr_selector.slice(step, step + MAX).join(',')
                    ...
                    + 'background-image: url(' + cssImageRelativePath + ')}';
            }
        }
        else {
        ...
        if (images[zero]) {
            for (i = 0, length = images[zero].length; i < length; i++) {
                ...
                for (j = 0, count = current.cls.length; j < count; j++) {
                    x_ = current.cls[j].position[0] + -x;
                    y_ = current.cls[j].position[1] + -y;
                    if (scale) {
                        x_ = x_ * scale;
                        y_ = y_ * scale;
                    }
