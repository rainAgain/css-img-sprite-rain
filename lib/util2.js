...
_.touch = function (path, mtime) {
    ...
    if (mtime instanceof Date) {
       ...
       console.log('invalid argument [mtime]');
    }
...
_.write = function (path, data, charset, append) {
    if (!_exists(path) && _.pathinfo(path).dirname) {
    ...
    function normalize(pattern) {
        ...
        switch (type) {
            ...
            case '[object RegExp]':
                return pattern;
            default:
               console.log('invalid regexp [' + pattern + '].');
        }
    }
...
_.del = function (rPath, include, exclude) {
    ...
    if (rPath && _.exists(rPath)) {
        ...
        if (/^(?:\w:)?\/$/.test(path)) {
           console.log('unable to delete directory [' + rPath + '].');
        }
...
_.copy = function (rSource, target, include, exclude, uncover, move) {
    ...
    if (source) {
       ...
       console.log('unable to copy [' + rSource + ']: No such file or directory.');
    }
    return removedAll;
};
                            ...
                            fs
                                ...
                                .on('error', function (err) {
                                    if (callback) {
                                       ...
                                       console.log('extract tar file [' + tmp + '] fail, error [' + err + ']');
                                    }
                                })
                                .on('end', function () {
    ...
    _.download(url, function (err) {
        if (err) {
            if (opt.error) {
               ...
               console.log('unable to download component [' +
...
_.nohup = function (cmd, options, callback) {
    ...
    if (IS_WIN) {
        ...
        return exec('cscript.exe /nologo "' + file + '"', options, function (error, stdout) {
            if (stdout != '0') {
               console.log('exec command[' + cmd + '] fail.');
            }
            fs.unlinkSync(file);
        ...
        return exec('nohup ' + cmd + ' > /dev/null 2>&1 &', options, function (error, stdout) {
            if (error !== null) {
               console.log('exec command[' + cmd + '] fail, stdout [' + stdout + '].');
            }
            if (typeof callback === 'function') {
                callback();
            }
