...
module.exports = function (buffer, obj) {
    ...
    var curWorkingDir = '',
        curWorkingDirCwd = process.cwd(),
        // 将content转化成string
        content = buffer.toString("utf-8"),
        res = cssParser(content, path.join(obj.cssSrc ? curWorkingDir : curWorkingDirCwd, obj.cssSrc)),
        images = {},
        cssRealOut = obj.cssDesDir ?
            path.join(obj.cssDesDir ? curWorkingDir : curWorkingDirCwd, obj.cssDesDir) :
            path.join(obj.cssSrc ? curWorkingDir : curWorkingDirCwd, path.dirname(obj.cssSrc)),
        content2 = res.content,
        cssFileName = path.basename(obj.cssSrc).split(".")[0],
        imgRealOutputDir = obj.imgDesDir ? path.join(obj.imgDesDir ? curWorkingDir : curWorkingDirCwd, obj.imgDesDir) : cssRealOut;
