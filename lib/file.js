...
function addHash(path, file) {
    var rExt = file.rExt,
        qRExt = util.escapeReg(rExt),
        qExt = util.escapeReg(file.ext),
        hash = file.getHash(),
        onnector,
...
function getDomainsByPath(path) {
    var domain = fis.config.get('roadmap.domain', {}),
        value = [];
    if (typeof domain === 'string') {
        value = domain.split(/\s*,\s*/);
    } else if (util.is(domain, 'Array')) {
        value = domain;
    } else {
        util.map(domain, function (pattern, domain) {
            if ((pattern === 'image' && util.isImageFile(path)) || util.glob(pattern, path)) {
                if (typeof domain === 'string') {
                    value = domain.split(/\s*,\s*/);
                } else if (util.is(domain, 'Array')) {
                    value = domain;
...
var File = Object.derive(function () {
    ...
    exists: function () {
        return util.exists(this.realpath);
    },
    isText: function () {
        return this._isText;
    },
    ...
    getMtime: function () {
        return util.mtime(this.realpath);
    },
    isFile: function () {
        return util.isFile(this.realpath);
    },
    isDir: function () {
        return util.isDir(this.realpath);
    },
    ...
    getContent: function () {
        if (typeof this._content === 'undefined') {
            this._content = util.read(this.realpath, this.isText());
        }
    ...
    getHash: function () {
        if (typeof this._md5 === 'undefined') {
            Object.defineProperty(this, '_md5', {
                value: util.md5(this.getContent()),
                writable: false
            });
        }
        return this._md5;
    ...
    getBase64: function (prefix) {
        ...
        if (prefix) {
            prefix = 'data:' + util.getMimeType(this.rExt) + ';base64,';
        } else {
            prefix = '';
        }
        return prefix + util.base64(this._content);
    },
    getId: function () {
        return this.id;
    },
    ...
    addSameNameRequire: function (ext) {
        var path;
        if (util.isFile(this.realpathNoExt + ext)) {
            ...
            for (var key in map) {
                if (map.hasOwnProperty(key)) {
                    ...
                    if (rExt === ext && util.isFile(this.realpathNoExt + oExt)) {
...
module.exports.wrap = function (file) {
    if (typeof file === 'string') {
        ...
        console.log('unable to convert [' + (typeof file) + '] to [File] object.');
    }
};
