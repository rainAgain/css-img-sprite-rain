...
var file2 = require("./file.js");
var CACHE_DIR;

var exports = module.exports = function (file) {
    if (!CACHE_DIR) {
        console.log('uninitialized compile cache directory.');
    }
    file = file2.wrap(file);
    if (!file.realpath) {
        console.log('unable to compile [' + file.subpath + ']: Invalid file realpath.');
    }
    console.log('compile [' + file.realpath + '] start');
    if (file.isFile()) {
        if (file.useCompile && file.ext && file.ext !== '.') {
            ...
            if (file.useCache && cache.revert(revertObj)) {
                ...
                file.setContent(util2.read(file.realpath));
                process(file);
            ...
            file.setContent(file.isText() ? util2.read(file.realpath) : util2.fs.readFileSync(file.realpath));
        }
    ...
    file.compiled = true;
    console.log('compile [' + file.realpath + '] end');
    embeddedUnlock(file);
    return file;
};
...
exports.setup = function (opt) {
    ...
    if (opt) {
        util2.map(settings, function (key) {
...
var map = exports.lang = (function () {
    var keywords = ['require', 'embed', 'uri', 'dep', 'jsEmbed'],
        LD = '<<<', RD = '>>>',
        qLd = util2.escapeReg(LD),
        qRd = util2.escapeReg(RD),
        map = {
            reg: new RegExp(
...
function extCss(content, callback) {
    ...
    callback = callback || function (m, comment, url, last, filter) {
            if (url) {
                var key = isInline(util2.query(url)) ? 'embed' : 'uri';
                if (m.indexOf('@') === 0) {
                    if (key === 'embed') {
            ...
            if ($1) {//<script>
                ...
                if (tag === 'link') {
                    ...
                    m = m.replace(/(\s(?:data-)?href\s*=\s*)('[^']+'|"[^"]+"|[^\s\/>]+)/ig, function (_, prefix, value) {
                        if ((isCssLink || isImportLink) && isInline(util2.query(value))) {
                            if (isCssLink) {
                    ...
                    m = m.replace(/(\s(?:data-)?src\s*=\s*)('[^']+'|"[^"]+"|[^\s\/>]+)/ig, function (m, prefix, value) {
                        var key = isInline(util2.query(value)) ? 'embed' : 'uri';
                    ...
                    if (tag == 'img') {
                        ...
                        m = m.replace(/(\ssrcset\s*=\s*)('[^']+'|"[^"]+"|[^\s\/>]+)/ig, function (m, prefix, value) {
                            var info = util2.stringQuote(value);
...
function pipe(file, type, ext, keep) {
    var key = type + ext;
    util2.pipe(key, function (processor, settings, key) {
        settings.filename = file.realpath;
        var content = file.getContent();
        try {
            console.log('pipe [' + key + '] start');
            var result = processor(content, file, settings);
            console.log('pipe [' + key + '] end');
            if (keep) {
...
function embeddedCheck(main, embedded) {
    main = file2.wrap(main).realpath;
    embedded = file2.wrap(embedded).realpath;
    if (main === embedded) {
            ...
            try {
                switch (type) {
                    ...
                    case 'jsEmbed':
                        ...
                        if (info.file) {
                            f = info.file;
                        } else if (util2.isAbsolute(info.rest)) {
                            f = file(info.rest);
                        }
                        if (f && f.isFile()) {
